...
<%
	...
	try{
		...
		String dbbirth = request.getParameter("birth");
	 	...
	 	pstmt = conn.prepareStatement("select 이름 from 회원 where ID = ?");	//ID 검색해서 이미 있는 거면
	 	...
	 	if(rs.next()){
		    ...
		    //System.out.println(result);
		 	...
		 	pstmt = conn.prepareStatement("insert into 회원(ID, PW, 이름, 연락처, 생일) values(?,?,?,?,?)");
		    pstmt.setString(1,dbID);
		    pstmt.setString(2,dbPW);
		    pstmt.setString(3,dbname);
		    pstmt.setString(4,dbtel);
		    pstmt.setString(5,dbbirth);
		    pstmt.executeUpdate();
		    
		    
		    pstmt = conn.prepareStatement("select 이름 from 회원 where ID = ?");
		    pstmt.setString(1,dbID);
		    
		    rs=pstmt.executeQuery();
		    
		    if(rs.next()){
		    	System.out.println("회원가입요");
