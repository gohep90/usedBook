...
public class FindId extends AppCompatActivity {
        ...
        OKbtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(name.getText().toString().equals("")){
                    name.requestFocus();
                    Toast.makeText(FindId.this, "ID를 입력하세요.", Toast.LENGTH_SHORT).show();
                }else if(tel.getText().toString().equals("")){
                    tel.requestFocus();
                    Toast.makeText(FindId.this, "전화번호를 입력하세요.", Toast.LENGTH_SHORT).show();
                }else{
                    asyncTaskCall();
                }
            }
    ...
    public class MyAsyncTask extends AsyncTask<String,Integer,String> {
        ...
        private String recvList() { //데이터 보내고 받아오기!!
            ...
            prop.setProperty("name",name.getText().toString());   //키값 "temGoods"으로 보낼 값 tempGoods을 보낸다.(사실 버퍼에 넣는거)
            ...
            try{
                ...
                url=new URL("http://" + ip + ":8080/usedBook/findID.jsp");
    ...
    private void jsonParserList(String recv) {  //다른 쓰레드로 이렇게 넘겨 주겠다
        ...
        try{
            ...
            JSONArray jArr =json.getJSONArray("ID");
            json=jArr.getJSONObject(0);
            if(!json.getString("id").equals("")) {
                Toast.makeText(FindId.this,"너의 아이디는 : " + json.getString("id"), Toast.LENGTH_SHORT).show();
                finish();   //수정되면 액티비티 종료
            }else{
                Toast.makeText(FindId.this, "비밀번호가 틀렸습니다.", Toast.LENGTH_SHORT).show();
            }
            //Toast.makeText(Login.this, "됐다", Toast.LENGTH_SHORT).show();
        }catch(JSONException e){
            e.printStackTrace();
            Toast.makeText(FindId.this, "실패", Toast.LENGTH_SHORT).show();
        }
